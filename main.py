...
def RegisterVoter(numCandidates, old_window):
    ...
    if(numCandidates >= 2):
        ...
        ent = tk.Entry(window)
        ent.pack()
        tk.Button(window,text="Enter", command=lambda:	StartVoting(int(ent.get()), vote, EM, BB, mac_rkey, rsa_ukey, window)).pack()
    else: 
        # Creates an error message if invalid candidates
        tk.Label(old_window, text='Need more than 1 candidate', fg='red').pack()

# Third window
def StartVoting(voterID, vote, EM, BB, mac_rkey, rsa_ukey, old_window):
    '''
